...
data class RowData_v1Ret(val x: Int, val y: Int, val z: List<Int>, val a: Int, val b: Int)
...
class cybtldr_parse {
    ...
    fun parse4ByteValueLittleEndian(buf:List<Int>): Int {
        return (parse2ByteValueLittleEndian(buf)) or
    ...
    @ExperimentalStdlibApi
    fun CyBtldr_FromAscii(bufSize:Int,buffer:List<String>) : Triple<Int,List<Int>,Int>{
        var i = 0
        var rowSize = 0
        ...
        if ((bufSize and 1) == 1) // Make sure even number of bytes
        {
            ...
            for(index in 0..bufSize/2){
                hexData[index] = (buffer[i*2].hexToInt() shl 4) or buffer[(i*2) +1].hexToInt()
                i += 1
            }
    ...
    fun CyBtldr_OpenDataFile(fileName: String) : Int{
        try {
    ...
    fun CyBtldr_ParseCyacdFileVersion(fileName: String, bufSize: Int,header: List<String>): Pair<Int,Int>{
        ...
        if (fileName.substring(index).lowercase() == ".cyacd2") {
            ...
            if (CYRET_SUCCESS == err) {
                version = (header[0].hexToInt() shl  4) or header[1].hexToInt();
                if (version == 0) {
                    err = CYRET_ERR_DATA
                }
            }
    ...
    fun CyBtldr_ParseHeader(bufSize:Int, buffer:List<String>): Pair<Pair<Int,Int>,Pair<Int,Int>>{
        var err = CYRET_SUCCESS
        var chksum = 0
        var siliconId = 0
        var siliconRev = 0
    ...
    fun CyBtldr_ParseHeader_v1(bufSize: Int,buffer: List<String>): Pair<Triple<Int,Int,Int>,Triple<Int,Int,Int>>{
        var err = CYRET_SUCCESS
        var siliconId = 0;
        var siliconRev = 0;
        var chksum = 0;
    ...
    //int bufSize, Uint8List buffer) {
    fun CyBtldr_ParseRowData(bufSize: Int,buffer: List<String>):Pair<Triple<Int,Int,Int>,Triple<List<Int>,Int,Int>> {
        ...
        if (bufSize <= MIN_SIZE) {
            ...
            var (errT, hexData, hexSize) = CyBtldr_FromAscii(bufSize - 1,
                buffer.subList(1, buffer.size))
                err = errT
    ...
    fun CyBtldr_ParseRowData_v1(bufSize: Int,buffer: String): RowData_v1Ret{
        ...
        if (bufSize <= MIN_SIZE) {
            ...
            var (err, hexData, hexSize) = CyBtldr_FromAscii(bufSize - 1, listOf(buffer.substring(1, 2)));
    ...
    fun CyBtldr_ParseAppStartAndSize_v1(appStartIn:Int, appSizeIn:Int, lineIndex:Int): Triple<Int,Int,Int>{
